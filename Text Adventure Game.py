...
#a global variable to determine how many attempts the user has to pick the correct option
attemptCounter = -1 
#To change the properties of different pieces of text 
#for example print(color.BOLD + 'pancakes' + color.END) prints pancakes but in bold
class color:
    BOLD = '\033[1m'
    END = '\033[0m'
    RED = '\033[91m'
    BLUE = '\033[94m'
    CYAN = '\033[96m'
#function that makes text bold, x is the text you want to be in bold
def bold(x):
...
def takeDmg(attacker, defender, enemy):
    ...
    if defender['stats']['hp'] <= 0:
        ...
        if hero['name'] in loser:
            ...
            blue(loser)
            hero['xp'] += tutorialEnemy['reward']
            hero['money'] += tutorialEnemy['reward']
...
#the function for path1 which branches out into different endings/scenarios
def path1():
    print("The choice has been made.")
    print()
    print("(EVERYTHING IS DARK)")
#the function for path2 which brances out into different ending / scenarios
def path2():
    global attemptCounter
    attemptCounter += 4
    print("The choice has been made.")
